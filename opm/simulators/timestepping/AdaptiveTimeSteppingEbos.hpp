...
namespace Opm {
    ...
    public:
        ...
        AdaptiveTimeSteppingEbos(const UnitSystem& unitSystem,
            ...
            , minTimeStep_(timeUnitConversion(unitSystem, EWOMS_GET_PARAM(TypeTag, double, SolverMinTimeStep))) // 1e-12;
        ...
        static void registerParameters()
        {
            ...
            EWOMS_REGISTER_PARAM(TypeTag, double, SolverMinTimeStep,
                                 "The minimum size of a time step in days for field and hours for lab. If a step cannot converge without getting cut below this step size the simulator will stop");
    ...
    protected:
        void init_(const UnitSystem& unitSystem)
        {
            ...
            if (control == "pid") {
                ...
                double minTimeStepReducedByIterations = timeUnitConversion(unitSystem, nonDimensionalMinTimeStepIterations);
        ...
        double timeUnitConversion(const UnitSystem& unitSystem, double value)
        {
            // the min time step can be reduced by the newton iteration numbers
