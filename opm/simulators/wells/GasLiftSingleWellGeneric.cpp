...
getInitialRatesWithLimit_() const
{
    ...
    if (auto [rates, alq] = computeInitialWellRates_(); rates) {
        ...
        //bool group_limited = (*limited_rates).limit_type == LimitedRates::LimitType::group;
        //if( (*limited_rates).water_is_limited && group_limited)
        //    return {std::nullopt, this->orig_alq_};
...
runOptimizeLoop_(bool increase)
{
    ...
    while (!state.stop_iteration && (++state.it <= this->max_iterations_)) {
        ...
        bool group_limited = (*temp_rates).limit_type == LimitedRates::LimitType::group;
        
        //if ((*temp_rates).water_is_limited && group_limited)
        //    break;
