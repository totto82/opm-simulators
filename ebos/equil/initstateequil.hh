...
private:
    ...
    void updateCellProps_(const Vanguard& vanguard)
    {
        ...
        for (; elemIt != elemEndIt; ++elemIt) {
            ...
            cellZSpan_[elemIdx] = cellZSpan(element);
            cellZMinMax_[elemIdx] = cellZMinMax(element);
        }
    }

    double cellCenterDepth(const Element& element) const
    {
    ...
    std::pair<double,double> cellZSpan(const Element& element) const
    {
        typedef typename Element::Geometry Geometry;
        static constexpr int zCoord = Element::dimension - 1;
        double bot = 0.0;
        ...
        return std::make_pair(top/4, bot/4);
    }

    std::pair<double,double> cellZMinMax(const Element& element) const
    {
        typedef typename Element::Geometry Geometry;
        static constexpr int zCoord = Element::dimension - 1;
        const Geometry geometry = element.geometry();
